...
**Result:** A seamless integration of Structure (1) + Style (2). Photorealistic, high quality.`;
  ...
  const runBatchGeneration = async (
    ...
    const totalImages = combinations.length * imageCount * inputImages.length;
    const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
    
    for (const syncedInputImage of inputImages) {
        for (const combo of combinations) {
            for (let i = 0; i < imageCount; i++) {
                ...
                try {
                    ...
                    const contentParts = [];
                    if (syncedInputImage) contentParts.push(dataUrlToPart(syncedInputImage));
  ...
  const handleGenerateImage = async () => {
    ...
    if (generationMode === 'batch') {
        ...
        if (imagesToProcess.length === 0) {
            setError('Vui lòng tải lên ít nhất một ảnh đầu vào để tạo hàng loạt.');
            setIsLoading(false);
            return;
        }
        ...
        const totalImages = combinations.length * imageCount * imagesToProcess.length;

        setBatchProgress({ current: 0, total: totalImages });
        
        const synchronizedSeed = seed;
  ...
  const totalBatchImagesCount = useMemo(() => {
    ...
    const imagesToProcess = batchInputImages.length > 0 ? batchInputImages : (originalInputImage ? [originalInputImage] : []);
    if (imagesToProcess.length === 0) return 0;
    
    const combinations = getCombinations(batchState);
    const hasVariations = combinations.length > 1 || (combinations.length > 0 && Object.keys(combinations[0]).length > 0);
    if (!hasVariations) return 0;
    
    return combinations.length * imageCount * imagesToProcess.length;
  ...
  const canGenerate = useMemo(() => {
    ...
    if (generationMode === 'single') {
        ...
        return totalBatchImagesCount > 0;
    }
  }, [isLoading, isMetaMode, keepShape, isNewBananaMode, isBananaFlowMode, isCTAIProMode, generationMode, prompt, originalInputImage, totalBatchImagesCount]);
                            ...
                            <section className="space-y-5">
                                ...
                                {generationMode === 'batch' && (
                                    <section>
                                        ...
                                        <p className="text-xs text-zinc-500 dark:text-zinc-500 mb-3">Nếu bạn thêm ảnh ở đây, các tùy chọn hàng loạt sẽ được áp dụng cho từng ảnh. Nếu không, chúng sẽ được áp dụng cho ảnh đầu vào chính ở trên.</p>
